  ...
  async resolveTicket(req, res) {
    try {
      ...
      if (!ticket.assignee.equals(req.user.id)) {
        return res
          .status(403)
          .json({ error: "Only assigned agent can resolve ticket" });
      }

      if (ticket.status === "Closed") {
        return res
          .status(400)
          .json({ error: "Cannot resolve a closed ticket" });
      }
  ...
  async closeTicket(req, res) {
    try {
      ...
      if (!ticket.ticket_raised_by.equals(req.user.id)) {
        return res
          .status(403)
          .json({ error: "Only ticket raiser can close the ticket" });
      }
  ...
  async rejectResolution(req, res) {
    try {
      ...
      if (ticket.status !== "Resolved") {
        return res
          .status(400)
          .json({ error: "Can only reject resolved tickets" });
      }
      ...
      if (!ticket.ticket_raised_by.equals(req.user.id)) {
        return res
          .status(403)
          .json({ error: "Only ticket raiser can reject resolution" });
      }
