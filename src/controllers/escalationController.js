...
class EscalationController {
  ...
  async checkEscalationStatus(ticket) {
    try {
      ...
      const threshold = await EscalationThreshold.findOne({
        priority: ticket.priority,
      });
      if (!threshold) {
        throw new Error(
          `No threshold defined for priority: ${ticket.priority}`
        );
      }
      ...
      if (!ticket.escalation.iEscalated) {
        ...
        const escalationAge =
          (Date.now() - new Date(ticket.escalation.escalatedAt).getTime()) /
          (1000 * 60 * 60);
    ...
    try {
      ...
      for (const ticket of tickets) {
        try {
          ...
          const { needsEscalation, reason, level, escalationTeam } =
            await this.checkEscalationStatus(ticket);
          ...
          if (!ticket.escalation.isEscalated) {
            ...
            if (hoursSinceLastEscalation >= 24) {
              ...
              if (!escalationTeam) {
                throw new Error(
                  `No escalation team found for level ${nextLevel}`
                );
              }
  ...
  async manualEscalate(req, res) {
    try {
      ...
      await ticket.save();
      await EmailService.sendEscalationUserEmail(req.user.email, ticket);
