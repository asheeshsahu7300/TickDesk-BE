...
class TicketController {
    ...
    async createTicket(req, res) {
        try {
            const { category, subcategory, description, priority, attachments,assignee } = req.body;
            ...
            const ticket = await Ticket.create({
                ...
                status: 'Open' // Assignee will be auto-assigned by pre-save hook
            });
            ...
            const conversation = await ticketConversation.create({
                ...
                sender_type: 'User',
                sender_role: 'user',
                message: description || '', // Ensure `message` is not undefined
    ...
    async getTicketById(req, res) {
        try {
            const ticket = await this._findAndVerifyTicketAccess(req.params.id, req.user);
            if (!ticket) {
    ...
    async addComment(req, res) {
        try {
            ...
            const senderRole = ticket.assignee.equals(req.user._id) ? 'assignee' : 'user';
    ...
    async updateStatus(req, res) {
        try {
            const { status, resolution_description, closure_reason, assurance } = req.body;
            const ticket = await this._findAndVerifyTicketAccess(req.params.id, req.user);
